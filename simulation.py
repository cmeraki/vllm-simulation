...
def orchestrate_conversation(model_id: str, uri: str, conversation_id: int, num_total_turns: int) -> None:
    """
    Starts a chat between a simulated user and an LLM

    Args
        - model_id {str}:           Name of the model to call
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_id', type=str, required=False, default='David-Xu/Mistral-7B-Instruct-v0.2', help='Model name that is called for inference')
    ...
    parser.add_argument('-r', type=float, required=False, default=4, help='Number of requests per second')
    ...
    logger.info(f'Model id: {model_id}, model uri: {complete_uri}')
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for arrival, num_total_turns in zip(arrival_rates, num_turns):
            ...
            f = partial(orchestrate_conversation, model_id=model_id, uri=complete_uri, conversation_id=idx, num_total_turns=num_total_turns)
            executor.submit(f)
            idx += 1
